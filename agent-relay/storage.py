...
Claim, heartbeat, terminal submission, and recovery each run in one atomic
transaction.  On PostgreSQL they lock the task rows they change (claims use
``FOR UPDATE SKIP LOCKED`` so workers never wait on each other's tasks); on
SQLite the transaction's writer lock serializes them.
"""
...
def create_task(sender_id: str, recipient_id: str, input_text: str, idempotency_key: str | None) -> dict[str, str]:
    # Serializing task creation per sender makes the sender-scoped idempotency
    # check and unique constraint one operation even when two API processes race.
    with immediate_transaction() as db:
